...
- **While a label is resolving, nothing shows the id.** A read cell or view panel renders a skeleton until `resolveOption` lands: `Region 1` is meaningless to whoever is reading the screen. If the resolve returns nothing or fails, the value reads `Unknown` and keeps the id in the tooltip, so a dangling reference looks like an error rather than like data. Identical resolves that are in flight at the same moment are shared, so a table of 50 rows pointing at one country makes one request, not 50.
