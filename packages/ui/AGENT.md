...
Group fields into form sections with `group`. It takes **any** title, and `RecordForm` renders one section per group in the order the groups first appear — so a profile form can read "Organization information", "Brand assets", "Contact & address", "Localization & units" instead of the default "General". Ungrouped fields fall under "General". Per-field `description` fills the About/info panel beside the form (the reference implementation is the Organization Profile page at `/organization/profile`, a read-only view with an Edit button that opens the standard Cancel + Save footer — `readOnly` + `onEdit` on `RecordForm`).
