...
**Form layout is declared, not styled** (full write-up: docs `/form-layout`). A form is a grid of section cards: `sectionColumns={1|2|3}` flows the cards across and they wrap onto as many rows as they need; `sections` declares them (order, `span: "full"`, a description line). Also a `form.sectionColumns` key in `VuiProvider`. **Every card is the same two columns, one field per row**: `[i] Label *` then the control, with hairlines between the columns and rows. Never stack a label above its control, and never add a per-field column count: a form is made wider by putting cards side by side, not by cramming fields. A field's `description` becomes the tooltip on its label, on top of the Info panel on full-page forms.
