...
export function generateStaticParams() {
  return COLLECTIONS.guides.entries.map((e) => ({ slug: e.slug }));
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const entry = entryBySlug("guides", slug);
  if (!entry) return {};
  return pageMeta({
    title: entry.title,
...
export default async function GuidesDetailPage({
  ...
  if (!entry) notFound();
  return (
    <EntryPage entry={entry} collection="guides" backLabel="Guides" backHref="/guides/" />
  );
}
