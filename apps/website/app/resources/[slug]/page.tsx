...
export function generateStaticParams() {
  return COLLECTIONS.resources.entries.map((e) => ({ slug: e.slug }));
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const entry = entryBySlug("resources", slug);
  if (!entry) return {};
  return pageMeta({
    title: entry.title,
...
export default async function ResourcesDetailPage({
  ...
  if (!entry) notFound();
  return (
    <EntryPage entry={entry} collection="resources" backLabel="Resources" backHref="/resources/" />
  );
}
