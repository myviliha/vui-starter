...
useEffect(() => {
  fetch("/api/tenant/branding")
    ...
    .then(setBrand); // { name, tagline, logoUrl, … }
      ...
      <H2>Docs login</H2>
      <P>
        <code>NEXT_PUBLIC_DOCS_EMAIL</code> and{" "}
        <code>NEXT_PUBLIC_DOCS_PASSWORD_HASH</code> put <code>/docs</code> behind
        a sign-in form. Set both and readers need the credentials; leave either
        unset and the docs are open, which is how the public demo runs. The
        password is an argon2id hash, generated by{" "}
        <code>pnpm --filter backoffice docs-password</code>, so no plain-text
        password lives in the environment or the bundle. Full write-up, including
        what a client-side gate can and cannot protect, is in{" "}
