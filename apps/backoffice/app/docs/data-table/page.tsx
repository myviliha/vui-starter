  ...
  return (
    ...
    />
      ...
      <Ul>
        ...
        <li><strong>Row Edit</strong> (<code>showEdit</code>): the one toggle that doesn&apos;t default to <code>true</code>. It follows your fields, so a list where nothing is marked <code>editable</code> shows no row pencil and no Edit button on the view panel. Opening that form would only show an empty body and a Save that writes nothing. Pass <code>showEdit={`{false}`}</code> to hide Edit on a table that does have editable fields, or <code>showEdit</code> to force it on. Use <code>onEdit</code> when you want the action to stay but navigate to your own edit route instead.</li>
        <li><strong>Trash mode</strong> (<code>showTrash</code>, off by default): adds a <strong>Trash</strong> toggle left of Filter that flips the same table between live and soft-deleted rows. RecordView is display-only — the host supplies the trashed rows (<code>trashedData</code> in client mode, or the <code>trash: true</code> flag on the <code>ServerQuery</code> in <code>manual</code>/<code>fetcher</code> mode) and persists restores via <code>onRestore(rows)</code>. Restore mirrors delete: a per-row Restore icon and bulk <em>&quot;Restore N selected&quot;</em>, each with a confirm. It&apos;s wired across the demo tables (Branches, Organizations, Users, the System reference tables and more) — client tables get it from <code>useClientFilter</code> (which routes deletes into Trash), and the Users server table via the <code>trash</code> query flag.</li>
      </Ul>
      ...
      <Ul>
        ...
        <li><code>multiple</code>: <strong>multi-select</strong> (many-to-many). The field&apos;s value becomes a <code>string[]</code>; the form renders a searchable multi-select with removable chips (from static <code>options</code> or async <code>loadOptions</code>), and the read cell shows up to <code>maxChipsInCell</code> (default 3) labels then &quot;+N&quot;. Async fields add <code>resolveOptions(values)</code> (batch companion to <code>resolveOption</code>). <code>required</code> ⇒ at least one. Reference: Markets → Post Codes.</li>
