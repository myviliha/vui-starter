...
**Status:** slices 0, 1, 4 and most of 2 shipped (1.50.0 to 1.52.0); slice 3 open · **Written:** 2026-08-06 · **Updated:** 2026-08-06 after the
...
## Slice 0 — The config spine (shipped, 1.50.0)
...
## Slice 1 — Form footer and actions (shipped, 1.50.0)
...
## Slice 2 — Form body composition (mostly shipped, 1.52.0)
...
Three additions, smallest first, because the first two probably cover most real cases. **The first
two shipped in 1.52.0**, with two changes of shape worth recording: in a label-beside-control grid
there is no difference between spanning two columns and spanning the row, so the field prop became
`fullWidth` rather than `span: 1 | 2 | "full"`; and slots landed as a `formSlots` prop rather than
entries in the `fields` array, to keep that array a pure data contract for the table, filter panel and
import/export. The third is still open and still waiting on a real case:
...
## Slice 3 — Page frames and the shell (open, the last one)
...
## Slice 4 — Behaviour (shipped, 1.51.0)
...
| Slice                    | Version | What a host writes                                                                                                                      |
| ------------------------ | ------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| 0, config spine          | 1.50.0  | `VuiProvider`, `vuiPreset`, `defineConfig`, `mergeConfig`, `useVuiConfig`                                                               |
| 1, footer actions        | 1.50.0  | `formActions`, `renderFooter`, `FormAction`, `FormActionContext`                                                                        |
| 4, behaviour             | 1.51.0  | `behaviour` (`rowClick`, `closeOnSave`, `flashMs`, `confirmDelete`, `confirmDiscardWhenDirty`), `userConfigurable`, `useVuiPreferences` |
| 2, form body             | 1.52.0  | `formSlots`, `fullWidth` (sections with tabs/accordion still open)                                                                      |
| 3, page frames and shell | open    | nothing yet                                                                                                                             |

The user-preference layer is wired live in the demo: `VuiProvider` mounts in `(app)/layout.tsx` and
Settings has a _Data tables_ section that changes `rowClick`, `flashMs` and `confirmDelete` per
browser. That is the check on whether "even in the preconfigured theme, the end user can configure and
change" is true rather than claimed.
